...
class RV32I (
  ...
  switch (control_unit.io_ctrl.next_pc_select) {
    ...
    // Add this part for JAL and JALR, RV32I file, Updated ----------------<
    is(NEXT_PC_SELECT.ALU_OUT_ALIGNED) {
      io_pc.pc_wdata := alu.io_alu.result & "hFFFFFFFE".U
      // we are making the least significant bit into 0
    }
  }
  ...
  switch(control_unit.io_ctrl.reg_write_sel) {
    ...
    // added for load instructions -----------------------------<
    is(REG_WRITE_SEL.MEM_OUT_SIGN_EXTENDED) {
      switch(control_unit.io_ctrl.data_be) {
        is("b0001".U) { //this is for LB
          // we are sign extending the byte into 32 bits
          io_reg.reg_write_data := Cat(Fill(24, io_data.data_rdata(7)), io_data.data_rdata(7, 0))
        }
        is("b0011".U) { //this is for LH
          // we are sign extending the half word into 32 bits
          io_reg.reg_write_data := Cat(Fill(16, io_data.data_rdata(15)), io_data.data_rdata(15, 0))
        }
        is("b1111".U) { //this is for LW
    ...
    is(REG_WRITE_SEL.MEM_OUT_ZERO_EXTENDED) {
      switch(control_unit.io_ctrl.data_be) {
        is("b0001".U) { //this is for LBU
          // we are zero extending the byte into 32 bits
          io_reg.reg_write_data := Cat(Fill(24, 0.U), io_data.data_rdata(7, 0))
        }
        is("b0011".U) { //this is for LHU
          // we are zero extending the half word into 32 bits
          io_reg.reg_write_data := Cat(Fill(16, 0.U), io_data.data_rdata(15, 0))
        }
      }
    }
  }
